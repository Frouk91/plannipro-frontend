    ...
    <div style={{ minHeight:"100vh",background:"linear-gradient(135deg,#1a1d27 0%,#16213e 50%,#0f3460 100%)",display:"flex",alignItems:"center",justifyContent:"center",fontFamily:"'DM Sans','Segoe UI',sans-serif" }}>
      <div style={{ width:"100%",maxWidth:420,padding:"0 24px" }}>
        ...
        <div style={{ background:"#fff",borderRadius:20,padding:36,boxShadow:"0 25px 60px rgba(0,0,0,0.4)" }}>
          ...
          <div style={{ marginBottom:8 }}>
            ...
            <div style={{ position:"relative" }}>
              <input type={showPassword?"text":"password"} value={password} onChange={e=>{setPassword(e.target.value);setError("");}} onKeyDown={e=>e.key==="Enter"&&handleLogin()} placeholder="••••••••"
              ...
              <button onClick={()=>setShowPassword(!showPassword)} style={{ position:"absolute",right:12,top:"50%",transform:"translateY(-50%)",background:"none",border:"none",cursor:"pointer",fontSize:16,color:"#9ca3af" }}>{showPassword?"🙈":"👁"}</button>
            </div>
          </div>
          ...
          <button onClick={handleLogin} disabled={loading} style={{ width:"100%",padding:"13px 0",borderRadius:10,border:"none",background:"linear-gradient(135deg,#4f46e5,#7c3aed)",color:"#fff",fontSize:15,fontWeight:700,cursor:"pointer",marginTop:16,boxShadow:"0 4px 15px rgba(79,70,229,0.4)",opacity:loading?0.7:1 }}>
        ...
        <div style={{ background:"rgba(255,255,255,0.08)",borderRadius:16,padding:20,marginTop:20 }}>
              ...
              <button key={u.id} onClick={()=>{setEmail(u.email);setPassword(u.password);setError("");}}
                ...
                <div style={{ display:"flex",alignItems:"center",gap:8 }}>
                  ...
                  <div>
                    ...
                    <div style={{ color:u.role==="admin"?"#f59e0b":u.role==="manager"?"#818cf8":"#9ca3af",fontSize:10 }}>
          ...
          <p style={{ color:"#6b7280",fontSize:11,textAlign:"center",margin:"12px 0 0" }}>Cliquez sur un compte pour remplir automatiquement</p>
        </div>
      </div>
    </div>
  );
...
// PANNEAU ADMIN
// ─────────────────────────────────────────────
function AdminPanel({ agents, onAgentAdded, onPasswordReset, showNotif }) {
  ...
  async function handleAddAgent() {
    ...
    try {
      ...
      const data = await res.json();
      if (data.agent) {
        const initials = (newAgent.first_name[0]+(newAgent.last_name[0]||"")).toUpperCase();
        onAgentAdded({
          id: Date.now(), name: `${newAgent.first_name} ${newAgent.last_name}`,
          email: newAgent.email, password: newAgent.password,
          role: newAgent.role, team: newAgent.team, avatar: initials
        });
        setAddModal(false);
        setNewAgent({ first_name:"",last_name:"",email:"",password:"",role:"agent",team:"" });
        showNotif("Agent ajouté avec succès ✅");
      } else {
  ...
  return (
    <div style={{ padding:24 }}>
      <div style={{ display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:24 }}>
        <div>
          <h2 style={{ margin:0,fontSize:18,fontWeight:700,color:"#111827" }}>⚙️ Panneau d'administration</h2>
          <p style={{ margin:"4px 0 0",fontSize:13,color:"#6b7280" }}>{agents.length} agents enregistrés</p>
        </div>
        <button onClick={()=>setAddModal(true)} style={{ background:"#4f46e5",color:"#fff",border:"none",borderRadius:8,padding:"10px 18px",cursor:"pointer",fontSize:14,fontWeight:600 }}>
          + Ajouter un agent
        </button>
      </div>
        ...
        <table style={{ width:"100%",borderCollapse:"collapse" }}>
          <thead>
            <tr style={{ background:"#f9fafb",borderBottom:"1px solid #e5e7eb" }}>
              <th style={{ padding:"12px 16px",textAlign:"left",fontSize:12,color:"#6b7280",fontWeight:600 }}>AGENT</th>
              <th style={{ padding:"12px 16px",textAlign:"left",fontSize:12,color:"#6b7280",fontWeight:600 }}>EMAIL</th>
              <th style={{ padding:"12px 16px",textAlign:"left",fontSize:12,color:"#6b7280",fontWeight:600 }}>ÉQUIPE</th>
              <th style={{ padding:"12px 16px",textAlign:"left",fontSize:12,color:"#6b7280",fontWeight:600 }}>RÔLE</th>
              <th style={{ padding:"12px 16px",textAlign:"left",fontSize:12,color:"#6b7280",fontWeight:600 }}>MOT DE PASSE</th>
              <th style={{ padding:"12px 16px",textAlign:"left",fontSize:12,color:"#6b7280",fontWeight:600 }}>ACTIONS</th>
            </tr>
          </thead>
          <tbody>
            {agents.map(agent=>(
              <tr key={agent.id} style={{ borderBottom:"1px solid #f3f4f6" }}>
                <td style={{ padding:"12px 16px" }}>
                  <div style={{ display:"flex",alignItems:"center",gap:10 }}>
                    ...
                    <span style={{ fontWeight:600,color:"#111827",fontSize:14 }}>{agent.name}</span>
                  </div>
                </td>
                <td style={{ padding:"12px 16px",fontSize:13,color:"#6b7280" }}>{agent.email}</td>
                <td style={{ padding:"12px 16px",fontSize:13,color:"#374151" }}>{agent.team}</td>
                <td style={{ padding:"12px 16px" }}>
                  <span style={{ background:agent.role==="admin"?"#fef3c7":agent.role==="manager"?"#eef2ff":"#f0fdf4",color:agent.role==="admin"?"#92400e":agent.role==="manager"?"#4338ca":"#166534",padding:"3px 10px",borderRadius:20,fontSize:11,fontWeight:600 }}>
                ...
                <td style={{ padding:"12px 16px",fontSize:13,color:"#9ca3af",fontFamily:"monospace" }}>
                  {"•".repeat(agent.password?.length||8)}
                </td>
                <td style={{ padding:"12px 16px" }}>
                  <button onClick={()=>{setResetModal(agent);setNewPassword("");}} style={{ background:"#f3f4f6",border:"none",borderRadius:6,padding:"6px 12px",cursor:"pointer",fontSize:12,color:"#374151",fontWeight:600 }}>
                    🔑 Réinitialiser MDP
                  </button>
                </td>
              </tr>
      ...
      {/* Modal ajouter agent */}
      {addModal&&<div style={{ position:"fixed",inset:0,background:"rgba(0,0,0,0.4)",display:"flex",alignItems:"center",justifyContent:"center",zIndex:1000 }}>
        <div style={{ background:"#fff",borderRadius:16,padding:32,width:460,boxShadow:"0 20px 60px rgba(0,0,0,0.2)" }}>
          ...
          <div style={{ display:"grid",gridTemplateColumns:"1fr 1fr",gap:12,marginBottom:12 }}>
            {[{k:"first_name",l:"Prénom",p:"Jean"},{k:"last_name",l:"Nom",p:"Dupont"}].map(f=>(
              <div key={f.k}>
                <label style={{ display:"block",fontSize:12,fontWeight:600,color:"#374151",marginBottom:4 }}>{f.l}</label>
                <input value={newAgent[f.k]} onChange={e=>setNewAgent(prev=>({...prev,[f.k]:e.target.value}))} placeholder={f.p}
          ...
          <div style={{ marginBottom:12 }}>
            ...
            <input value={newAgent.email} onChange={e=>setNewAgent(prev=>({...prev,email:e.target.value}))} placeholder="jean.dupont@entreprise.fr" type="email"
              style={{ width:"100%",padding:"10px 14px",borderRadius:8,border:"1px solid #d1d5db",fontSize:14,boxSizing:"border-box",outline:"none" }}/>
          </div>
          <div style={{ marginBottom:12 }}>
            <label style={{ display:"block",fontSize:12,fontWeight:600,color:"#374151",marginBottom:4 }}>Mot de passe (min 8 caractères)</label>
            <input value={newAgent.password} onChange={e=>setNewAgent(prev=>({...prev,password:e.target.value}))} placeholder="motdepasse123" type="text"
              style={{ width:"100%",padding:"10px 14px",borderRadius:8,border:"1px solid #d1d5db",fontSize:14,boxSizing:"border-box",outline:"none" }}/>
          </div>
          <div style={{ display:"grid",gridTemplateColumns:"1fr 1fr",gap:12,marginBottom:20 }}>
            <div>
              <label style={{ display:"block",fontSize:12,fontWeight:600,color:"#374151",marginBottom:4 }}>Équipe</label>
              <input value={newAgent.team} onChange={e=>setNewAgent(prev=>({...prev,team:e.target.value}))} placeholder="Tech, RH, Ventes..."
                style={{ width:"100%",padding:"10px 14px",borderRadius:8,border:"1px solid #d1d5db",fontSize:14,boxSizing:"border-box",outline:"none" }}/>
            </div>
            <div>
          ...
          <div style={{ display:"flex",gap:10 }}>
            <button onClick={()=>setAddModal(false)} style={{ flex:1,padding:10,borderRadius:8,border:"1px solid #e5e7eb",background:"#fff",cursor:"pointer",fontSize:14 }}>Annuler</button>
            <button onClick={handleAddAgent} disabled={loading} style={{ flex:1,padding:10,borderRadius:8,border:"none",background:"#4f46e5",color:"#fff",cursor:"pointer",fontSize:14,fontWeight:600,opacity:loading?0.7:1 }}>
      ...
      {resetModal&&<div style={{ position:"fixed",inset:0,background:"rgba(0,0,0,0.4)",display:"flex",alignItems:"center",justifyContent:"center",zIndex:1000 }}>
        <div style={{ background:"#fff",borderRadius:16,padding:32,width:420,boxShadow:"0 20px 60px rgba(0,0,0,0.2)" }}>
          <h2 style={{ margin:"0 0 8px",fontSize:18,fontWeight:700 }}>🔑 Réinitialiser le mot de passe</h2>
          <p style={{ color:"#6b7280",fontSize:13,margin:"0 0 20px" }}>Nouveau mot de passe pour <strong>{resetModal.name}</strong></p>
          <input value={newPassword} onChange={e=>setNewPassword(e.target.value)} placeholder="Nouveau mot de passe (min 8 caractères)" type="text"
            style={{ width:"100%",padding:"12px 16px",borderRadius:8,border:"1px solid #d1d5db",fontSize:14,boxSizing:"border-box",outline:"none",marginBottom:20 }}/>
          <div style={{ background:"#fef3c7",border:"1px solid #fde68a",borderRadius:8,padding:"10px 14px",fontSize:12,color:"#92400e",marginBottom:20 }}>
            ⚠️ Notez bien ce mot de passe pour le communiquer à l'agent.
          </div>
          <div style={{ display:"flex",gap:10 }}>
            <button onClick={()=>setResetModal(null)} style={{ flex:1,padding:10,borderRadius:8,border:"1px solid #e5e7eb",background:"#fff",cursor:"pointer",fontSize:14 }}>Annuler</button>
            <button onClick={()=>{
              if(newPassword.length>=8){
                onPasswordReset(resetModal.id, newPassword);
                setResetModal(null);
                showNotif("Mot de passe réinitialisé ✅");
              } else {
                showNotif("Minimum 8 caractères requis", "error");
              }
            }} style={{ flex:1,padding:10,borderRadius:8,border:"none",background:"#f59e0b",color:"#fff",cursor:"pointer",fontSize:14,fontWeight:600 }}>
              Confirmer
            </button>
          </div>
        </div>
      ...
      <main style={{ flex:1,overflow:"auto" }}>
        <div style={{ background:"#fff",borderBottom:"1px solid #e5e7eb",padding:"16px 28px",display:"flex",alignItems:"center",justifyContent:"space-between" }}>
          <div>
            ...
            <p style={{ margin:"2px 0 0",fontSize:13,color:"#6b7280" }}>
              {view==="planning"?`${MONTHS_FR[month]} ${year}`:view==="validations"?(isManager?`${pendingRequests.length} demande(s) en attente`:`${myRequests.length} demande(s)`):view==="admin"?`${agents.length} agents`:""}
            </p>
          </div>
        </div>
        ...
        {view==="admin"&&isAdmin&&(
          <AdminPanel
            ...
            onPasswordReset={(id,pwd)=>setAgents(prev=>prev.map(a=>a.id===id?{...a,password:pwd}:a))}
          />
        )}
          ...
          <div style={{ padding:24 }}>
            <div style={{ display:"flex",alignItems:"center",gap:12,marginBottom:16,flexWrap:"wrap" }}>
              ...
              <div style={{ marginLeft:"auto",display:"flex",gap:8,flexWrap:"wrap" }}>
                {teams.map(t=><button key={t} onClick={()=>setFilterTeam(t)} style={{ padding:"5px 12px",borderRadius:20,border:"1px solid",fontSize:12,cursor:"pointer",fontWeight:500,background:filterTeam===t?"#4f46e5":"#fff",color:filterTeam===t?"#fff":"#6b7280",borderColor:filterTeam===t?"#4f46e5":"#e5e7eb" }}>{t}</button>)}
              </div>
            </div>
            <div style={{ display:"flex",gap:8,marginBottom:12,flexWrap:"wrap" }}>
              {LEAVE_TYPES.map(t=><button key={t.id} onClick={()=>setSelectedLeaveType(t.id)} style={{ padding:"5px 12px",borderRadius:20,border:"2px solid",fontSize:12,cursor:"pointer",fontWeight:600,background:selectedLeaveType===t.id?t.color:t.bg,color:selectedLeaveType===t.id?"#fff":t.color,borderColor:t.color }}>{t.label}</button>)}
            </div>
            ...
            <div style={{ background:"#fff",borderRadius:12,border:"1px solid #e5e7eb",overflow:"auto" }}>
              <table style={{ width:"100%",borderCollapse:"collapse",tableLayout:"fixed" }}>
                ...
                <tbody>
                  {filteredAgents.filter(a=>a.role!=="admin").map(agent=>(
                    <tr key={agent.id} style={{ borderBottom:"1px solid #f3f4f6" }}>
          ...
          <div style={{ padding:24 }}>
            {isManager?(
              <>
                ...
                {requests.filter(r=>r.status!=="pending").length>0&&<><h3 style={{ fontSize:15,fontWeight:700,color:"#111827",margin:"24px 0 16px" }}>📋 Historique</h3>{requests.filter(r=>r.status!=="pending").map(req=><RequestCard key={req.id} req={req} isManager={true}/>)}</>}
              </>
            ):(
              <>
                <div style={{ background:"#eef2ff",border:"1px solid #c7d2fe",borderRadius:12,padding:16,marginBottom:24,fontSize:13,color:"#4338ca" }}>💡 Sélectionnez des dates dans le planning pour envoyer une demande au manager.</div>
