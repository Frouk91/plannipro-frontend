...
function isExceptional(leave, leaveTypes) {
  if (!leave) return false;
  if (leave.is_exceptional) return true;
  // Fallback : vérifier dans la liste des types de congés par code
  if (leaveTypes && leave.code) {
    const lt = leaveTypes.find(t => t.code === leave.code);
    return !!(lt && lt.is_exceptional);
  }
  return false;
}
    ...
    <div style={{ minHeight: "100vh", background: "#060818", display: "flex", alignItems: "center", justifyContent: "center", fontFamily: "'Outfit',sans-serif", position: "relative", overflow: "hidden" }}>
      ...
      <style>{`
        ...
        .shooting-star {
          ...
          background: linear-gradient(90deg, rgba(255,255,255,0.95), rgba(165,180,252,0.6), transparent);
        ...
        .shooting-star::before {
          ...
          box-shadow: 0 0 6px 2px rgba(165,180,252,0.85), 0 0 14px 5px rgba(99,102,241,0.45);
        }
        .star-dot {
          position: absolute;
          border-radius: 50%;
        ...
        {cells.map((d, i) => {
          ...
          return (
            <button key={i} onClick={() => !isDisabled && onChange(dateStr)}
              style={{
                width: "100%", aspectRatio: "1", borderRadius: 8, border: "none", cursor: isDisabled ? "not-allowed" : "pointer", fontSize: 12, fontWeight: isSelected ? 700 : 400,
...
function PlanningApp({ currentUser, onLogout }) {
    ...
    const loadAstreintes = async () => {
      try {
        ...
        if (response.ok) {
          ...
          const astreinteObj = {};
          data.forEach(a => {
            // Convertir la date ISO en format YYYY-MM-DD simple
            const dateStr = typeof a.date_key === 'string'
              ? a.date_key.split('T')[0]
              : new Date(a.date_key).toISOString().split('T')[0];
            const key = `${a.team_name}|${a.row_type}|${dateStr}`;
            astreinteObj[key] = a;
          });
  ...
  const saveAstreinte = useCallback(async (teamName, rowType, dateKey, agentId) => {
    console.log('🔵 saveAstreinte appelée:', { teamName, rowType, dateKey, agentId, token: !!token });
    try {
      console.log('📡 Envoi POST à:', `${API}/astreintes`);
      const response = await fetch(`${API}/astreintes`, {
        method: 'POST',
        headers: {
      ...
      if (response.ok) {
        ...
        console.log('✅ Astreinte sauvegardée:', data);
        return data;
      } else {
  ...
  useEffect(() => {
    document.title = validationBadge > 0 ? `🔴 Mon Planning` : "Mon Planning";
  }, [validationBadge]);
        ...
        <div style={{ padding: "22px 20px 18px", borderBottom: "1px solid rgba(255,255,255,0.06)", position:"relative" }}>
          <div style={{ display: "flex", alignItems: "center", gap: 11 }}>
            <div style={{ flex:1, minWidth:0 }}>
              <div style={{ fontSize: 18, fontWeight: 800, color: "#f8fafc", letterSpacing: "-0.6px", fontFamily: "'Space Grotesk',sans-serif", lineHeight:1.1 }}>
                Quadient
              </div>
              <div style={{ fontSize: 10, color: "#475569", fontWeight: 500, marginTop: 3, letterSpacing:"0.2px" }}>
                Gestion des congés {new Date().getFullYear()}
              </div>
            </div>
          </div>
          {/* Live sync indicator */}
        ...
        <nav style={{ padding: "10px 10px", flex: 1, display:"flex", flexDirection:"column", gap:2 }}>
          {navItems.map((item, i) => {
            ...
            const navIcons = {
              ...
              exceptional: <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><polygon points="12 2 15.09 8.26 22 9.27 17 14.14 18.18 21.02 12 17.77 5.82 21.02 7 14.14 2 9.27 8.91 8.26 12 2"/></svg>,
                                  ...
                                  Array.from({ length: daysInMonth }, (_, i) => {
                                    ...
                                    return <td key={i}
                                      ...
                                      {filterMode !== "astreinte" && leave && !wk && !isFer && (
                                        isExceptional(leave, leaveTypes) ? (
  ...
  const allExceptional = useMemo(() => {
    ...
    agents.forEach(agent => {
      ...
      Object.entries(agentLeaves).forEach(([dateKey, leave]) => {
        if (dateKey.endsWith("__presence")) return;
        if (!leave || !leave.leaveId) return;
        if (!exceptionalCodes.has(leave.code) && !isExceptional(leave, leaveTypes)) return;
