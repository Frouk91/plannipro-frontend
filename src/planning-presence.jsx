  ...
  async function loadLeaves(ltList, tok, y, m) {
    try {
      ...
      leavesData.forEach(l => {
        ...
        const lt = (ltList||leaveTypes).find(t=>t.code===l.leave_type_code||t.id===l.leave_type_id)||{label:"Congé",color:"#6366f1",bg:"#eef2ff"};
        // Remplir chaque jour de la période
        const start = new Date(l.start_date);
        const end = new Date(l.end_date);
        for (let d = new Date(start); d <= end; d.setDate(d.getDate()+1)) {
          const k = `${d.getFullYear()}-${String(d.getMonth()+1).padStart(2,"0")}-${String(d.getDate()).padStart(2,"0")}`;
          leavesMap[l.agent_id][k] = {...lt, bg:lt.bg||hexToLight(lt.color), status:l.status||"approved"};
        }
      });
      setLeaves(leavesMap);
