                      ...
                      <tr key={agent.id} style={{ borderBottom: "1px solid #f8fafc" }}>
                        ...
                        {weekDays.map((d, i) => {
                          ...
                          return <td key={i}
                            ...
                            {leave && !wk && !isFer && <div style={{ width: "calc(100% - 6px)", height: 30, margin: "0 3px", background: leave.status === "pending" && leave.agentId !== currentUser.id ? hexToLight(leave.color) : leave.color, borderRadius: 6, display: "flex", alignItems: "center", justifyContent: "center", boxShadow: leave.status === "pending" && leave.agentId !== currentUser.id ? "none" : `0 2px 8px ${leave.color}50`, border: leave.status === "pending" && leave.agentId !== currentUser.id ? `1.5px dashed ${leave.color}` : "none" }}>
                              <span style={{ fontSize: 10, color: leave.status === "pending" && leave.agentId !== currentUser.id ? leave.color : "#fff", fontWeight: 700 }}>{leave.status === "pending" && leave.agentId !== currentUser.id ? "?" : leaveAbbr(leave.label)}</span>
                            </div>}
